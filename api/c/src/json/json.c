...
void json_init_hooks(json_hook_t* hooks)
{
...
{
	json_t* node = (json_t*)cJSON_malloc(sizeof(json_t));
...
void json_destroy(json_t *c)
{
	...
	while (c)
	{
		...
		if (!(c->type&JSON_IsReference) && c->valuestring) cJSON_free(c->valuestring);
...
static char *print_value(json_t *item,int depth,int fmt)
{
	...
	{
	case JSON_NULL:	out=cJSON_strdup("null");	break;
	case JSON_FALSE:	out=cJSON_strdup("false");break;
	case JSON_TRUE:	out=cJSON_strdup("true"); break;
...
void   json_object_add(json_t *object,const char *string,json_t *item)	{if (!item) return; if (item->string) cJSON_free(item->string);item->string=cJSON_strdup(string);json_array_add(object,item);}
...
void   json_object_replace(json_t *object,const char *string,json_t *newitem){int i=0;json_t *c=object->child;while(c && cJSON_strcasecmp(c->string,string))i++,c=c->next;if(c){newitem->string=cJSON_strdup(string);json_array_replace(object,i,newitem);}}
...
json_t *json_string(const char *string)	{json_t *item=cJSON_New_Item();if(item){item->type=JSON_STRING;item->valuestring=cJSON_strdup(string);}return item;}
...
json_t *json_dup(json_t *item,int recurse)
{
	...
	if (item->valuestring)	{newitem->valuestring=cJSON_strdup(item->valuestring);	if (!newitem->valuestring)	{json_destroy(newitem);return 0;}}
	if (item->string)		{newitem->string=cJSON_strdup(item->string);			if (!newitem->string)		{json_destroy(newitem);return 0;}}
