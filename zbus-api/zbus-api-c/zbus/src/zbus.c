...
static int consumer_register(consumer_t* self, int timeout){
	...
	msg_set_subcmd(msg, CREATE_MQ);
	
	json = json_object();
	json_object_addstr(json, "mqName", self->mq);
	json_object_addstr(json, "accessToken", self->acc_token);
	sprintf(mode, "%d", self->mode);
	json_object_addstr(json, "mqMode", mode);
	params = json_dump(json);
	
	msg_set_json_body(msg, params);
	free(params);
	json_destroy(json);
...
void caller_set_token(caller_t* self, char* value){
	if(self->token){
		free(self->token);
	}
...
rpc_t* rpc_set_token(rpc_t* self, char* value){
	caller_set_token(self->rpc, value);
	return self;
}
...
service_cfg_t* service_cfg_new(){
	service_cfg_t* self = malloc(sizeof(*self));
	memset(self, 0, sizeof(*self));
