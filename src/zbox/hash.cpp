...
hash_new(hash_ctrl_t *type, void *privdata) {
    hash_t *d = (hash_t*)zmalloc(sizeof(*d));
...
hash_expand(hash_t *d, unsigned long size){
    ...
    n.table = (hash_entry_t**)zcalloc(realsize*sizeof(hash_entry_t*));
    n.used = 0;
...
hash_add(hash_t *d, void *key, void *val){
    ...
    entry = (hash_entry_t*)zmalloc(sizeof(*entry));
...
hash_iter_new(hash_t *d){
    hash_iter_t *iter = (hash_iter_t*)zmalloc(sizeof(*iter));
