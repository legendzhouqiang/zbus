...
static inline void *
safe_malloc (size_t size, char *file, unsigned line){
    void *mem; 
    mem = calloc (1, size);
    if (mem == NULL) {
        fprintf (stderr, "FATAL ERROR at %s:%u", file, line);
...
ZBOX_EXPORT zthread_t
	zthread_new(thread_fn thread, void* args);
