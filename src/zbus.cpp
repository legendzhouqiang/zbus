...
zbus_new(int argc, char* argv[]){
	...
	self->broker_id = zstrdup(option(argc,argv,"-id","ZBUS01"));
	...
	assert(rc == 0);
	rc = zmq_setsockopt(self->socket, ZMQ_IDENTITY, self->broker_id, strlen(self->broker_id));
	assert(rc == 0);
